...
# en 1 seule commande supprime et relance le conteneur
# supprime le conteneur, l'image, nettoie le système et relance le conteneur
# pour le nom du conteneur, il faut remplacer sync-folders-app par le nom du conteneur
NOM_CONTAINER=bigmoletos/ync-folders-app
docker-compose down && docker stop $NOM_CONTAINER && docker rm $NOM_CONTAINER && docker rmi $NOM_CONTAINER:latest && docker system prune -f && docker-compose build --no-cache && docker-compose up -d && docker-compose logs -f
